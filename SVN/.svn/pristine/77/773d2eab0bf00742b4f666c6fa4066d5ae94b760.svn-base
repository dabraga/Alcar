...
/**
 * Rotina		:	ALRPCP07
 * Autor		:	Ectore Cecato - Totvs Jundiaí / Melhoria Ademar Jr.
 * Data			:	11/10/2013 - 21/05/2015
 * Descrição	:	Carta de Mistura
 * Módulo		:  	PCP
**/ 
User Function ALRPCP07()

	Local oReport := Nil
	Local cPerg   := "ALRPCP07"
...
Static Function ReportDef(cPerg)
	...
	Local oReport	:= Nil
	Local oOrdem	:= Nil
	Local cTitulo 	:= "Carta de Mistura"
	Local cAjuda  	:= "Permite imprimir a Carta de Mistura da Alcar."
	
	oReport := TReport():New("ALRPCP07", cTitulo, cPerg, {|oReport| ReportPrint(oReport)}, cAjuda)
...
Static Function ReportPrint(oReport)
	...
	cQuery += "	SB1B.B1_DESC AS DESCMP,SB1A.B1_COD AS CODPRO, D4_QTDEORI, C2_ZZIMP "
	...
	If Select("ALRPCP02") <> 0
		ALRPCP02->(dbCloseArea())
	Endif
	
	TcQuery cQuery Alias "ALRPCP02" New
	
	oReport:SetMeter(ALRPCP02->(LastRec()))
	
	ALRPCP02->(dbGoTop())
	
	While !ALRPCP02->(Eof())
		
		cCodProd := ALRPCP02->CODPRO
		
		If !Empty(ALRPCP02->C2_ZZIMP)
		
			If !MsgYesNo("OP "+ ALRPCP02->C2_NUM+ALRPCP02->C2_ITEM+ALRPCP02->C2_SEQUEN +" já impressa. Deseja imprimí-la novamente ?")
				ALRPCP02->(DbSkip())
				Loop
			EndIf 
		...
		cOp			:= ALRPCP02->C2_NUM+ALRPCP02->C2_ITEM+ALRPCP02->C2_SEQUEN
		aOpPai 		:= PesqOpPai(cOp, ALRPCP02->C2_PRODUTO)
		cQtdMist 	:= CalcMist(ALRPCP02->C2_QUANT, ALRPCP02->B1_ZZCAPAC, nKg, nQtdInf, @nFator)
		
		ImpCabec(oReport, @nLin, cQtdMist, ALRPCP02->C2_QUANT)
		
		ImpItem(oReport, @nLin, cOp, nKg, nFator, cQtdMist)
		
		ImpRodape(oReport, @nLin, aOpPai)
		
		DbSelectArea("SB1")
		SB1->(DbSetOrder(1))
		
		If SB1->(DbSeek(xFilial("SB1") + cCodProd))
			ImpInstm(oReport,@nLin)
		EndIf
	
		If !ALRPCP02->(Eof()) .And. nLin > 1510 
...
Static Function ImpCabec(oReport, nLin, cQtdMist, nTotMist)

	Local lImpCab := IIF(nLin == 0, .T., .F.)	 
	
	If nLin == 0 		
		nLin := 50		//Metade superior da página
	Else				
			
		If MV_PAR08 == 2
			nLin := 1510 	//Metade inferior da página
		Else
	...
	oReport:Say(nLin, 0900, "EMISSÃO: "+ DTOC(STOD(ALRPCP02->C2_EMISSAO)), 						oFont13)
	oReport:Say(nLin, 1500, cValToChar(nTotMist),														oFont13)
	oReport:Say(nLin, 1900, "ORDEM: "+ ALRPCP02->C2_NUM+ALRPCP02->C2_ITEM+ALRPCP02->C2_SEQUEN,	oFont13)
		
	nLin += 70
		
	oReport:Say(nLin, 0050, "PENEIRA: "+ ALRPCP02->B1_ZZPENEI, oFont13)
	...
	oReport:Say(nLin, 0050, "OBS: "+ ALRPCP02->B1_ZZOBS, oFont13)
		
	nLin += 70
		
	oReport:Say(nLin, 0050, "CÓD. ESTRUTURA: "+ AllTrim(ALRPCP02->C2_PRODUTO) +" "+ ;
		IIF(!Empty(ALRPCP02->CODIGO_RM), "| Cód. RM: "+ ALRPCP02->CODIGO_RM, "") + Space(5) +"DESCRIÇÃO: "+ AllTrim(ALRPCP02->DESCMIST), oFont13)
...
Static Function ImpItem(oReport, nLin, cOp, nKg, nFator, cQtdMist)
	...
	While !ALRPCP02->(Eof()) .And. ALRPCP02->C2_NUM+ALRPCP02->C2_ITEM+ALRPCP02->C2_SEQUEN == cOp
		...
		oReport:Say(nLin, 0150, ALRPCP02->D4_COD, oFont13)
	
		nLin2		:= 0
		
		aDescricao	:= ConvDesc(AllTrim(ALRPCP02->DESCMP))
		...
		If nKg > 0
			oReport:Say(nLin, 1450, Transform(Round(ALRPCP02->D4_QTDEORI * nFator, 3), "@E 999,999.999"), oFont13)
		Else
			oReport:Say(nLin, 1450, Transform(Round(ALRPCP02->D4_QTDEORI / nFator, 3), "@E 999,999.999"), oFont13)
		Endif
		...
		GravaImp(ALRPCP02->C2_NUM+ALRPCP02->C2_ITEM+ALRPCP02->C2_SEQUEN)
...
Static Function ImpRodape(oReport, nLin, aOpPai)
...
Static Function ImpInstm(oReport,nLin)
	...
	oReport:Say(nLin, 0030, "INSTRUÇÕES:", oFont13)
	
	nLinhas := MLCount(SB1->B1_ZZINSTM, 100)
	
	For nXi:= 1 To nLinhas
	
		cTxtLin := MemoLine(SB1->B1_ZZINSTM, 100, nXi)

        If ! Empty(cTxtLin)
    
           nLin += 50
           
           oReport:Say(nLin, 0030, cTxtLin, oFont13)
    
        EndIf
	
	Next nXi
	
	oReport:Box(nBoxLin,20,nBoxLin + ((nLinhas + 1) * 50), 2350)
...
PutSX1(cPerg, "08", "Cartas por folha","Cartas por folha","Cartas por folha",	"MV_PAR08", "N", 03, 0, 0, "C",,,,, 		"MV_PAR08","Uma","Uma","Uma","Duas","Duas","Duas",,,,,,,,,,, {"Informe a quantidade de cartas por folha"}, 	{"Informe a quantidade de cartas por folha"}, 	{"Informe a quantidade de cartas por folha"})*/
	...
	aAdd(aParBox, {1, "Carta por folha", Space(3), "@E999", "","","", 70, .T.})




Return ParamBox(aParBox, "Carta de Mistura",,,,,,,,cPerg, .T., .T.)